...
const featureItems = [
  {
    ...
    iconPath: (
      <MdOutlineArticle className="text-blue-600 light:text-blue-500 w-6 h-6" />
    ),
  },
  {
  ...
  },
  {
    ...
    iconPath: (
      <VscGraph className="text-blue-600 light:text-blue-500 w-6 h-6" />
    ),
  },
];
      ...
      {featureItems.map((item) => (
        <div key={item.title} className="flex m-2">
          ...
          <div>
            ...
            <p className="text-base text-balance max-w-9/10">
              {item.description}
            </p>
          </div>
        </div>
